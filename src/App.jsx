...
.error-message { background: #ffebee; color: #c62828; padding: 10px; border-radius: 4px; margin-bottom: 1rem; text-align: left; }
...
function AuthView({ onLogin }) {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      setError('Network error: Ensure backend is running and CORS is enabled.');
    } finally {
      setLoading(false);
    }
  };
...
function DashboardView({ user, onNavigate }) {
  return <div className="dashboard-container"><h2>Welcome back!</h2></div>;
}