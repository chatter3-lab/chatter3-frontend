...
export default function NotificationCenter({ user, API_URL, authFetch }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(!open)} aria-label="Notifications" style={{ background: 'none', border: 'none', cursor: 'pointer', position: 'relative', padding: 4 }}>
      ...
      {open && (
        <div role="dialog" aria-label="Notifications" style={{ position: 'absolute', top: '100%', right: 0, marginTop: 8, background: 'white', borderRadius: 12, boxShadow: '0 8px 30px rgba(0,0,0,.15)', width: 320, maxHeight: 400, overflow: 'auto', zIndex: 100 }}>
          ...
          {notifications.length === 0 ? (
            ...
            <div key={n.id} style={{ padding: '10px 16px', borderBottom: '1px solid #f3f4f6', background: n.read ? 'white' : '#f0f4ff', fontSize: '.85rem' }}>
              <div>{n.message}</div>
